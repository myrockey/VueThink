...
class wechatOauth{
    ...
    public function wechatConfig() {
        ...
        $wechat_config = array_merge($this->wechat_config,config('wechatlogin.oauth'));
    ...
    public function getSingleAccessToken($code) {
        ...
        //刷新refresh_token
        $access_token_url = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.$this->wechat_config['appid'].'&grant_type=refresh_token&refresh_token='.$access_token['refresh_token'];
        //转成对象
        $access_token2 = $this->https_request($access_token_url);
        if (isset($access_token2['errcode'])) {
            return '';
        }
