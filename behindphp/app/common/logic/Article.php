...
class Article extends LogicBase
{
     ...
     * 获取文章列表
     */
    public function getArticleList($where = [], $field = 'm.*', $order = '', $paginate = 0)
    {
    	return self::$articleModel->getList($where, $field.',articlecate.name as tidname,picture.path as thumb', $order,$paginate,[['articlecate','m.tid=articlecate.id','LEFT'],['picture','m.thumb_id=picture.id','LEFT']]);
    ...
    public function articleDel($where = [])
    {
        ...
        return self::$articleModel->deleteInfo($where,true) ? [RESULT_SUCCESS, '文章删除成功'] : [RESULT_ERROR, self::$articleModel->getError()];
    }
    ...
    return self::$articleModel->deleteAllInfo(['id'=>array('in',$ids)],true) ? [RESULT_SUCCESS, '文章删除成功'] : [RESULT_ERROR, self::$articleModel->getError()];
    }  
