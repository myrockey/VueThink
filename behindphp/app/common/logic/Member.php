...
//    	$data['is_inside'] = DATA_NORMAL;
    ...
    public function logout()
    {
    	...
    	session('[destroy]');
    	cookie('sys_key',null);
    	return [RESULT_SUCCESS, '注销成功', url('index/index')];
    }
     ...
     * 微信登录授权注册处理
     */
    public function regWechatHandle($userInfo = [])
    {
        if(empty($userInfo['openid'])) : return [RESULT_ERROR, '用户信息不存在']; endif;
        $username = md5($userInfo['openid']);
        $password = $username;
        $usermail = $username.'@qq.com';
        ...
        // 用户密码
        $data['nickname']  = addslashes($userInfo['nickname']);
        $data['sex']  = $userInfo['sex'];
        $data['userhome']  = $userInfo['country'].$userInfo['province'].$userInfo['city'];
        $data['userhead']  = $userInfo['headimgurl'];
        $data['leader_id'] = (isset($userInfo['pid']) && $userInfo['pid']) ? $userInfo['pid'] : 0;
    ...
    /**
     * 微信网页授权后，查询个人信息，判断是否会员过期，如果过期则，修改会员状态
     * 前台自动登录处理
     */
    public function loginAuto($member = [],$userInfo = [])
    {
        if($member['is_vip'] && $member['vip_expire_time'] < TIME_NOW){
            $data['is_vip'] = 0;
        }
        $data['last_login_ip'] = CLIENT_IP;
        ...
        //保持微信信息和数据库一致
        if(!empty($userInfo)){
            if(addslashes($member['nickname']) != addslashes($userInfo['nickname']))$member['nickname'] = $data['nickname'] = addslashes($userInfo['nickname']);
            if($member['sex'] != $userInfo['sex'])$member['sex'] = $data['sex'] = $userInfo['sex'];
            if($member['userhome'] != $userInfo['country'].$userInfo['province'].$userInfo['city'])$member['userhome'] = $data['userhome'] = $userInfo['country'].$userInfo['province'].$userInfo['city'];
            if($member['userhead'] != $userInfo['headimgurl'])$member['userhead'] = $data['userhead'] = $userInfo['headimgurl'];
...
//            point_controll($member['id'],'login',0);//登录增加经验值
        ...
        $lasturl = session('lasturl') ? urldecode(session('lasturl')) : url('member/index');
        return [RESULT_SUCCESS, '登录成功', $lasturl];

    }

    /**
     * 推荐码是否存在，是否有对应的会员ID，并且会员没有过期，
     */
    public function checkReommendCodeExists($reommendCode = ''){
        if(!$reommendCode) : return false; endif;
        $userid = deRecommendCode($reommendCode);
        $minfo = self::$memberModel->getInfo(['status'=>1,'id'=>$userid],'id,vip_expire_time');
        if($minfo && $minfo['vip_expire_time'] && ($minfo['vip_expire_time'] > time())) {
            return $minfo;
        }else{
            return false;
        }
    ...
    public function tologout()
    {
        ...
        session('[destroy]');
        cookie('sys_key',null);
        return [RESULT_SUCCESS, '请登录', url('member/index')];
    }

    /**
     * 普通游客体验7天会员
     */
    public function tofree7day($pinfo){
        $memberauth = session('member_auth');
        $data['id'] = $memberauth['member_id'];
        $data['is_vip'] = 1;
        $data['vip_expire_time'] = time() + 7*24*3600;
        $data['leader_id'] = $pinfo['id'];
        $data['grades'] = 1;

        if(self::$memberModel->setInfo($data)){
            $member = self::$memberModel->getInfo(['id'=>$data['id']],true);
            $auth = ['member_id' => $member['id'], 'last_login_time' => TIME_NOW];
            session('member_info', $member);
            session('member_auth', $auth);
            session('member_auth_sign', data_auth_sign($auth));
