        ...
        if($cacdata['code']>0){
        	...
        	if($data['do']['status']!=1){
        		...
        		if(strpos($m,'imzaker.com')!== false){
        			...
        			file_put_contents('./template/'.config('site_tpl').'/PC/Public/footer.html','<div class="n-foot"><div class="n-foot-bottom text-center"><p>Copyright © 2017 imzaker.com 版权所有 {$Think.CONFIG.web_site_icp}</p><p>{$Think.CONFIG.web_site_footer|html_entity_decode}</p></div></div>');
        		}
